...
test('play some games', async ({ page }) => {
  ...
  await expect(page.locator('button:has-text("PvC") >> nth=-1')).toHaveClass(
    /\bbg-white\b/
  )
  await expect(page.locator('button:has-text("PvP") >> nth=-1')).toHaveClass(
    /\bbg-blue-700\b/
  )
  ...
  await expect(
    page.locator('div.w-20.h-20:has-text("Player 1 (X)") >> nth=-1')
  ).toContainText(/[^\d]4\s*$/)
  await expect(
    page.locator('div.w-20.h-20:has-text("Tie") >> nth=-1')
  ).toContainText(/[^\d]6\s*$/)
  await expect(
    page.locator('div.w-20.h-20:has-text("Player 2 (O)") >> nth=-1')
  ).toContainText(/[^\d]5\s*$/)
  ...
  await expect(
    page.locator('div.w-20.h-20:has-text("Computer (O)") >> nth=-1')
  ).toContainText(/[^\d]1\s*$/)
  ...
  // switch to PvP mode
  await page.locator('button:has-text("PvP") >> nth=-1').click()
  ...
  // switch to CvP mode
  await page.locator('button:has-text("Computer vs Player") >> nth=-1').click()
  await page.waitForTimeout(500)
  await expect(
    page.locator('div.w-20.h-20:has-text("Player 1 (X)") >> nth=-1')
  ).toHaveClass(/\bborder-dashed\b/)
